  ...
  typeof window !== 'undefined'
    ? (() => {
        ...
        if (base && base.startsWith('//')) {
          try {
            return new URL(`${window.location.protocol}${base}`).origin;
          } catch {
            /* noop */
          }
        }
        if (base && base.length && !base.startsWith('/')) {
          try {
...
function detectBase64Content(content?: string | null): Base64Detection | null {
  ...
  if (trimmed.startsWith('data:')) {
    ...
    if (match) {
      ...
      const type = mime.startsWith('image/')
        ...
        ? 'audio'
        : 'document';
      return { type, mime, dataUrl: trimmed };
    }
  }

  const sanitized = trimmed.replace(/\s+/g, '');
  if (!/^[A-Za-z0-9+/=]+$/.test(sanitized) || sanitized.length < 60) {
    return null;
  }

  const createDetection = (
    mime: string,
    type: Base64Detection['type']
  ): Base64Detection => ({
    type,
    mime,
    dataUrl: `data:${mime};base64,${sanitized}`,
  });

  if (sanitized.startsWith('/9j/'))
    return createDetection('image/jpeg', 'image');
  if (sanitized.startsWith('iVBOR'))
    return createDetection('image/png', 'image');
  if (sanitized.startsWith('R0lGO'))
    return createDetection('image/gif', 'image');
  if (sanitized.startsWith('JVBER'))
    return createDetection('application/pdf', 'document');
  if (sanitized.startsWith('T2dn'))
    return createDetection('audio/ogg', 'audio');
  if (sanitized.startsWith('UklGR'))
    return createDetection('audio/wav', 'audio');
  if (sanitized.startsWith('SUQz') || sanitized.startsWith('AAAAGGZ0'))
    return createDetection('audio/mpeg', 'audio');

  return createDetection('application/octet-stream', 'document');
}

function looksLikeBase64(value?: string | null): boolean {
  if (!value) return false;
  const trimmed = value.trim();
  if (!trimmed) return false;
  if (trimmed.startsWith('data:')) return true;
  const sanitized = trimmed.replace(/\s+/g, '');
  return sanitized.length > 60 && /^[A-Za-z0-9+/=]+$/.test(sanitized);
}
